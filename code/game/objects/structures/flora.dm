...
/obj/structure/flora/tree/attackby(obj/item/W, mob/user, params, mob/living/user)
    ...
    var lumberjackskill = lumberjacker.mind.get_skill_level(/datum/skill/labor/lumberjacking)
    ...
    if (log_amount && (!(flags_1 & NODECONSTRUCT_1)))
        ...
        if (W.get_sharpness() > 0 && lumberjackskill > 0)
            ...
            user.visible_message("<span class='notice'>[user] begins to cut down [src] with [W].</span>",
                                 "<span class='notice'>I begin to cut down [src] with [W].</span>",
                                 "<span class='hear'>I hear the sound of sawing.</span>")
            if (do_after(user, 500 / lumberjackskill, target = src)) // 5 seconds with 20 force, 8 seconds with a hatchet, 20 seconds with a shard.
                user.visible_message("<span class='notice'>[user] fells [src] with the [W].</span>",
                                     "<span class='notice'>I fell [src] with the [W].</span>",
                                     "<span class='hear'>I hear the sound of a tree falling.</span>")
